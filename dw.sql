...
cpf_cliente NUMERIC(15) primary key,
...
CONSTRAINT fk_Produto foreign key (id_categoria) REFERENCES Categoria(id_categoria)
);
...
id_data INTEGER,
CONSTRAINT fk_Vendas1 foreign key (cpf_cliente) REFERENCES Clientes(cpf_cliente),
CONSTRAINT fk_Vendas2 foreign key (id_produto) REFERENCES Produto(id_produto),
CONSTRAINT fk_Vendas3 foreign key (id_data) REFERENCES Data(id_data)
);
...
CONSTRAINT fk_Compras1 foreign key (id_produto) REFERENCES Produto(id_produto),
CONSTRAINT fk_Compras2 foreign key (id_data) REFERENCES Data(id_data),
CONSTRAINT fk_Compras3 foreign key (id_fornecedor) REFERENCES
Fornecedor(id_fornecedor)
);
CREATE TABLE Fato_Lucratividade(
ano INTEGER,
mes INTEGER,
id_data INTEGER,
id_produto NUMERIC(10),
lucro NUMERIC(10,2),
CONSTRAINT fk_Lucratividade foreign key(id_data) REFERENCES Data(id_data),
CONSTRAINT fk_Lucratividade2 foreign key(id_produto) REFERENCES
Produto(id_produto)
);

ALTER TABLE Clientes ADD COLUMN ultima_venda DATETIME;

ALTER TABLE Data DROP COLUMN nome;

ALTER TABLE Fato_Vendas ADD COLUMN id_categoria INTEGER;

ALTER TABLE Fato_Vendas ADD CONSTRAINT fk_Vendas4 foreign key (id_categoria)
REFERENCES Categoria(id_categoria);

ALTER TABLE Fato_Vendas ADD COLUMN matricula INTEGER;

ALTER TABLE Fato_Vendas ADD CONSTRAINT fk_Vendas5 foreign key (matricula)
REFERENCES Funcionario(matricula);

ALTER TABLE Fato_Vendas ADD COLUMN gasto_media NUMERIC(10,2);

ALTER TABLE Fato_Vendas ADD COLUMN dia INTEGER;

ALTER TABLE Fato_Vendas ADD COLUMN dia_semana INTEGER;

ALTER TABLE Fato_Vendas ADD COLUMN mes INTEGER;

ALTER TABLE Fato_Vendas ADD COLUMN ano INTEGER;

ALTER TABLE Fato_Compras ADD COLUMN mes INTEGER;

ALTER TABLE Fato_Compras ADD COLUMN ano INTEGER;

CREATE VIEW FATO_ULTIMACOMPRA AS
SELECT CPF_CLIENTE, NOME, DATEDIFF(CURDATE(), ULTIMA_VENDA) 'DIA ULTIMA VENDA'
FROM DW.CLIENTES;

CREATE VIEW FATO_LUCRO AS
SELECT PROD_COMPRA2 ID_PROD, MES_COMPRA2 ANO_MES,
ANO_COMPRA2 ANO_NUMERO, DATAS2, IFNULL(COMPCOMP, 0) CUSTO,
IFNULL(VENDVEND, 0) RECEITA FROM
(SELECT COMP.PROD_COMPRA PROD_COMPRA2, COMP.MES_COMPRA
MES_COMPRA2, COMP.ANO_COMPRA ANO_COMPRA2, COMP.DATAS2
,SUM(COMP.COMPRAS) COMPCOMP FROM
(SELECT C.TB012_COD_PRODUTO PROD_COMPRA, C.TB012_017_DATA DATAS2,
MONTH(C.TB012_017_DATA) MES_COMPRA, YEAR(C.TB012_017_DATA) ANO_COMPRA,
(SUM(C.TB012_017_QUANTIDADE)*SUM(C.TB012_017_VALOR_UNITARIO)) COMPRAS
FROM DATAWAREHOUSE.TB012_017_COMPRAS C
GROUP BY C.TB012_COD_PRODUTO, MONTH(C.TB012_017_DATA),
YEAR(C.TB012_017_DATA), C.TB012_017_VALOR_UNITARIO) COMP
GROUP BY COMP.PROD_COMPRA, COMP.MES_COMPRA,
COMP.ANO_COMPRA) CMP LEFT JOIN
(SELECT VEND.PROD_VENDA PROD_VENDA2, VEND.MES_VENDA
MES_VENDA2, VEND.ANO_VENDA ANO_VENDA2, SUM(VEND.VENDAS)
VENDVEND FROM
(SELECT V.TB012_COD_PRODUTO PROD_VENDA, MONTH(V.TB010_012_DATA)
MES_VENDA, YEAR(V.TB010_012_DATA) ANO_VENDA,
(SUM(V.TB010_012_VALOR_UNITARIO)*SUM(V.TB010_012_QUANTIDADE)) VENDAS
FROM DATAWAREHOUSE.TB010_012_VENDAS V
GROUP BY V.TB012_COD_PRODUTO, MONTH(V.TB010_012_DATA),
YEAR(V.TB010_012_DATA), V.TB010_012_VALOR_UNITARIO) VEND
GROUP BY VEND.PROD_VENDA, VEND.MES_VENDA, VEND.ANO_VENDA) VND
ON VND.PROD_VENDA2 = CMP.PROD_COMPRA2 AND VND.ANO_VENDA2 =
CMP.ANO_COMPRA2 AND VND.MES_VENDA2 = CMP.MES_COMPRA2;

INSERT INTO DW.CATEGORIA (ID_CATEGORIA, NOME)
SELECT TB013_COD_CATEGORIA, TB013_DESCRICAO
FROM DATAWAREHOUSE.TB013_CATEGORIAS;

INSERT INTO DW.PRODUTO (ID_PRODUTO, NOME, ID_CATEGORIA)
SELECT TB012_COD_PRODUTO, TB012_DESCRICAO, TB013_COD_CATEGORIA
FROM DATAWAREHOUSE.TB012_PRODUTOS;

INSERT INTO DW.CLIENTES (CPF_CLIENTE, NOME, ULTIMA_VENDA)
SELECT TB010_CPF, TB010_NOME, NULL
FROM DATAWAREHOUSE.TB010_CLIENTES;

INSERT INTO DW.DATA (ID_DATA, DATA, DIA, MES, ANO)
SELECT TB010_012_COD_VENDA, TB010_012_DATA, DAY(TB010_012_DATA),
MONTH(TB010_012_DATA), YEAR(TB010_012_DATA)
FROM DATAWAREHOUSE.TB010_012_VENDAS;

INSERT INTO DW.FORNECEDOR (ID_FORNECEDOR, NOME)
SELECT TB017_COD_FORNECEDOR, TB017_NOME_FANTASIA
FROM DATAWAREHOUSE.TB017_FORNECEDORES;

INSERT INTO DW.FUNCIONARIO (MATRICULA, NOME)
SELECT TB005_MATRICULA, TB005_NOME_COMPLETO
FROM DATAWAREHOUSE.TB005_FUNCIONARIOS;

UPDATE DW.CLIENTES SET ULTIMA_VENDA =
(SELECT MAX(V.TB010_012_DATA)
FROM DATAWAREHOUSE.TB010_012_VENDAS V
WHERE V.TB010_CPF = CPF_CLIENTE
GROUP BY V.TB010_CPF);

INSERT INTO DW.FATO_COMPRAS (QUANTIDADE, VALOR, ID_PRODUTO, ID_DATA,
ID_FORNECEDOR, MES, ANO)
SELECT SUM(C.TB012_017_QUANTIDADE), SUM(C.TB012_017_VALOR_UNITARIO),
C.TB012_COD_PRODUTO, V.TB010_012_COD_VENDA, C.TB017_COD_FORNECEDOR,
MONTH(C.TB012_017_DATA), YEAR(C.TB012_017_DATA)
FROM DATAWAREHOUSE.TB012_017_COMPRAS C, DATAWAREHOUSE.TB010_012_VENDAS V
GROUP BY C.TB012_COD_PRODUTO;

INSERT INTO DW.FATO_COMPRAS (QUANTIDADE, VALOR, ID_PRODUTO, ID_DATA,
ID_FORNECEDOR, MES, ANO)
SELECT SUM(C.TB012_017_QUANTIDADE), SUM(C.TB012_017_VALOR_UNITARIO),
C.TB012_COD_PRODUTO, V.TB010_012_COD_VENDA, C.TB017_COD_FORNECEDOR, NULL,
YEAR(C.TB012_017_DATA)
FROM DATAWAREHOUSE.TB012_017_COMPRAS C, DATAWAREHOUSE.TB010_012_VENDAS V
GROUP BY C.TB012_COD_PRODUTO, YEAR(C.TB012_017_DATA);

INSERT INTO DW.FATO_VENDAS (QUANTIDADE, VALOR, CPF_CLIENTE, ID_PRODUTO, ID_DATA,
ID_CATEGORIA, GASTO_MEDIA, DIA, DIA_SEMANA, MES, ANO)
SELECT SUM(V.TB010_012_QUANTIDADE), SUM(V.TB010_012_VALOR_UNITARIO),
V.TB010_CPF, V.TB012_COD_PRODUTO, V.TB010_012_COD_VENDA, P.TB013_COD_CATEGORIA,
NULL, DAY(V.TB010_012_DATA), WEEKDAY(V.TB010_012_DATA), MONTH(V.TB010_012_DATA),
...
F.TB005_MATRICULA AND F.TB004_COD_LOJA = L.TB004_COD_LOJA
...
SELECT SUM(V.TB010_012_QUANTIDADE), SUM(V.TB010_012_VALOR_UNITARIO),
V.TB010_CPF, V.TB012_COD_PRODUTO, V.TB010_012_COD_VENDA, P.TB013_COD_CATEGORIA,
NULL, NULL, WEEKDAY(V.TB010_012_DATA), MONTH(V.TB010_012_DATA),
YEAR(V.TB010_012_DATA)
FROM DATAWAREHOUSE.TB010_012_VENDAS V, DATAWAREHOUSE.TB012_PRODUTOS P,
DATAWAREHOUSE.TB005_FUNCIONARIOS F, DATAWAREHOUSE.TB004_LOJAS L
WHERE P.TB012_COD_PRODUTO = V.TB012_COD_PRODUTO AND V.TB005_MATRICULA =
F.TB005_MATRICULA AND F.TB004_COD_LOJA = L.TB004_COD_LOJA
GROUP BY V.TB012_COD_PRODUTO, WEEKDAY(V.TB010_012_DATA),
MONTH(V.TB010_012_DATA), YEAR(V.TB010_012_DATA), L.TB004_COD_LOJA;

INSERT INTO DW.FATO_VENDAS (QUANTIDADE, VALOR, CPF_CLIENTE, ID_PRODUTO, ID_DATA,
ID_CATEGORIA, GASTO_MEDIA, DIA, DIA_SEMANA, MES, ANO)
SELECT SUM(V.TB010_012_QUANTIDADE), SUM(V.TB010_012_VALOR_UNITARIO),
V.TB010_CPF, V.TB012_COD_PRODUTO, V.TB010_012_COD_VENDA, P.TB013_COD_CATEGORIA,
NULL, NULL, NULL, MONTH(V.TB010_012_DATA), YEAR(V.TB010_012_DATA)
FROM DATAWAREHOUSE.TB010_012_VENDAS V, DATAWAREHOUSE.TB012_PRODUTOS P,
DATAWAREHOUSE.TB005_FUNCIONARIOS F, DATAWAREHOUSE.TB004_LOJAS L
WHERE P.TB012_COD_PRODUTO = V.TB012_COD_PRODUTO AND V.TB005_MATRICULA =
F.TB005_MATRICULA AND F.TB004_COD_LOJA = L.TB004_COD_LOJA
...
SELECT SUM(V.TB010_012_QUANTIDADE), SUM(V.TB010_012_VALOR_UNITARIO),
V.TB010_CPF, V.TB012_COD_PRODUTO, V.TB010_012_COD_VENDA, P.TB013_COD_CATEGORIA,
NULL, NULL, NULL, NULL, YEAR(V.TB010_012_DATA)
FROM DATAWAREHOUSE.TB010_012_VENDAS V, DATAWAREHOUSE.TB012_PRODUTOS P,
DATAWAREHOUSE.TB005_FUNCIONARIOS F, DATAWAREHOUSE.TB004_LOJAS L
WHERE P.TB012_COD_PRODUTO = V.TB012_COD_PRODUTO AND V.TB005_MATRICULA =
F.TB005_MATRICULA AND F.TB004_COD_LOJA = L.TB004_COD_LOJA
...
INSERT INTO DW.FATO_VENDAS (QUANTIDADE, VALOR, CPF_CLIENTE, ID_PRODUTO, ID_DATA,
ID_CATEGORIA, GASTO_MEDIA, DIA, DIA_SEMANA, MES, ANO)
SELECT SUM(V.TB010_012_QUANTIDADE) QUANTIDADE, SUM(V.TB010_012_VALOR_UNITARIO)
VALOR, V.TB010_CPF, V.TB012_COD_PRODUTO, V.TB010_012_COD_VENDA,
P.TB013_COD_CATEGORIA,
(V.TB010_012_VALOR_UNITARIO/DAYOFMONTH(LAST_DAY(V.TB010_012_DATA))) AS
MEDIA_POR_DIA, NULL, NULL, MONTH(V.TB010_012_DATA), YEAR(V.TB010_012_DATA)
FROM DATAWAREHOUSE.TB010_012_VENDAS V, DATAWAREHOUSE.TB012_PRODUTOS P,
DATAWAREHOUSE.TB005_FUNCIONARIOS F, DATAWAREHOUSE.TB004_LOJAS L
WHERE P.TB012_COD_PRODUTO = V.TB012_COD_PRODUTO AND V.TB005_MATRICULA =
F.TB005_MATRICULA AND F.TB004_COD_LOJA = L.TB004_COD_LOJA
GROUP BY V.TB010_CPF, MONTH(V.TB010_012_DATA)
ORDER BY V.TB010_012_VALOR_UNITARIO DESC LIMIT 30;

INSERT INTO DW.FATO_VENDAS (QUANTIDADE, VALOR, CPF_CLIENTE, ID_PRODUTO, ID_DATA,
ID_CATEGORIA, GASTO_MEDIA, DIA, DIA_SEMANA, MES, ANO)
SELECT SUM(V.TB010_012_QUANTIDADE) QUANTIDADE, SUM(V.TB010_012_VALOR_UNITARIO)
VALOR, V.TB010_CPF, V.TB012_COD_PRODUTO, V.TB010_012_COD_VENDA,
P.TB013_COD_CATEGORIA, (V.TB010_012_VALOR_UNITARIO/12) AS MEDIA_POR_MES, NULL, NULL,
NULL, YEAR(V.TB010_012_DATA)
FROM DATAWAREHOUSE.TB010_012_VENDAS V, DATAWAREHOUSE.TB012_PRODUTOS P,
DATAWAREHOUSE.TB005_FUNCIONARIOS F, DATAWAREHOUSE.TB004_LOJAS L
WHERE P.TB012_COD_PRODUTO = V.TB012_COD_PRODUTO AND V.TB005_MATRICULA =
F.TB005_MATRICULA AND F.TB004_COD_LOJA = L.TB004_COD_LOJA
GROUP BY V.TB010_CPF, YEAR(V.TB010_012_DATA)
ORDER BY V.TB010_012_VALOR_UNITARIO DESC LIMIT 30;

INSERT INTO DW.FATO_VENDAS (QUANTIDADE, VALOR, CPF_CLIENTE, ID_PRODUTO, ID_DATA,
ID_CATEGORIA, GASTO_MEDIA, DIA, DIA_SEMANA, MES, ANO, MATRICULA)
SELECT COUNT(TB010_012_DATA) QUANTIDADE, NULL, TB010_CPF, TB012_COD_PRODUTO,
TB010_012_COD_VENDA, NULL, NULL, DAY(TB010_012_DATA) DIA, WEEKDAY(TB010_012_DATA),
MONTH(TB010_012_DATA) MES, YEAR(TB010_012_DATA) ANO, TB005_MATRICULA MATRICULA
FROM DATAWAREHOUSE.TB010_012_VENDAS
...
ID_CATEGORIA, GASTO_MEDIA, DIA, DIA_SEMANA, MES, ANO, MATRICULA)
SELECT COUNT(TB010_012_DATA) QUANTIDADE, NULL, TB010_CPF, TB012_COD_PRODUTO,
TB010_012_COD_VENDA, NULL, NULL, NULL, NULL, MONTH(TB010_012_DATA) MES,
YEAR(TB010_012_DATA) ANO, TB005_MATRICULA MATRICULA
FROM DATAWAREHOUSE.TB010_012_VENDAS
GROUP BY TB005_MATRICULA, YEAR(TB010_012_DATA), MONTH(TB010_012_DATA);
...
ID_CATEGORIA, GASTO_MEDIA, DIA, DIA_SEMANA, MES, ANO, MATRICULA)
SELECT COUNT(TB010_012_DATA) QUANTIDADE, NULL, TB010_CPF, TB012_COD_PRODUTO,
TB010_012_COD_VENDA, NULL, NULL, NULL, NULL, NULL, YEAR(TB010_012_DATA) ANO,
TB005_MATRICULA MATRICULA
FROM DATAWAREHOUSE.TB010_012_VENDAS
GROUP BY TB005_MATRICULA, YEAR(TB010_012_DATA);

INSERT INTO DW.FATO_LUCRATIVIDADE (ANO, MES, ID_DATA, ID_PRODUTO, LUCRO)
SELECT F.ANO_NUMERO, F.ANO_MES, V.TB010_012_COD_VENDA, F.ID_PROD,
(F.RECEITA - F.CUSTO)
FROM FATO_LUCRO F, DATAWAREHOUSE.TB010_012_VENDAS V;

INSERT INTO DW.FATO_LUCRATIVIDADE (ANO, MES, ID_DATA, ID_PRODUTO, LUCRO)
SELECT F.ANO_NUMERO, NULL, V.TB010_012_COD_VENDA, F.ID_PROD,
(SUM(F.RECEITA)-SUM(F.CUSTO)) 'LUCRO ANUAL'
FROM FATO_LUCRO F, DATAWAREHOUSE.TB010_012_VENDAS V
GROUP BY F.ID_PROD, F.ANO_NUMERO;
...
INSERT INTO tb002_cidades VALUES(null, 'AL', 'Maceio');
...
INSERT INTO tb012_produtos VALUES(83, 8, 'Bonûs');
...
INSERT INTO tb010_012_vendas VALUES(null, 10000000000, 10,  4, '2011-05-11', 2, 17.31);
INSERT INTO tb010_012_vendas VALUES(null, 10000000000, 10,  4, '2011-02-11', 2, 38.89);
...
INSERT INTO tb012_017_compras VALUES(null, 10,  4, '2010-02-21', 3, 0.89);

INSERT INTO tb012_017_compras VALUES(null, 10,  4, '2011-02-01', 3, 0.89);
